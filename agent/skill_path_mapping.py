...
def map_skill_dir_for_backend(
    ...
    mount_candidates: list[tuple[Path, str]] = [
        (Path(mount["host_path"]), mount["container_path"]) for mount in mounts
    ]
    # get_skills_directory_mount() may replace the mounted host path with a
    # sanitized temp copy when the skills tree contains symlinks. Prompt
    # rendering still receives the original ~/.hermes/skills/... path, so also
    # match against the configured roots while reusing the mounted container
    # destination. This preserves the symlink-sanitized mount while avoiding a
    # false fallback to the host path in the agent prompt.
    try:
        from hermes_constants import get_hermes_home

        skills_dir = get_hermes_home() / "skills"
        for mount in mounts:
            if mount["container_path"].rstrip("/").endswith("/skills"):
                mount_candidates.append((skills_dir, mount["container_path"]))
                break
    except Exception as exc:  # noqa: BLE001 - path hinting must not break skills
        logger.debug("Could not add canonical skills root mapping: %s", exc)
    ...
    try:
        ...
        for mount in mounts:
            marker = "/external_skills/"
            container_path = mount["container_path"].rstrip("/")
            if marker not in container_path:
                continue
            try:
                idx = int(container_path.rsplit(marker, 1)[1])
            except ValueError:
                continue
            if 0 <= idx < len(external_dirs):
                mount_candidates.append((external_dirs[idx], mount["container_path"]))
    except Exception as exc:  # noqa: BLE001 - path hinting must not break skills
        logger.debug("Could not add external skills root mappings: %s", exc)

    # Longest-prefix match so a more specific mount (external_skills/<idx>) wins
    # over a parent that also matches.
    best: tuple[int, str] | None = None
    for mount_host, container_path in mount_candidates:
        try:
        ...
        if rel_posix == ".":
            ...
            mapped = posixpath.join(container_path, rel_posix)
