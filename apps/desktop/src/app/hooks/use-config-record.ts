...
// Preserve the existing optional-profile updater API while writing through to
// upstream's normalized per-profile key.
...
export const hermesConfigCacheWriter = (profile?: null | string) =>
  (next: HermesConfigRecord | undefined | ((prev: HermesConfigRecord | undefined) => HermesConfigRecord | undefined)) =>
    void setHermesConfigCache(next, profile)
